...
Recently, I contributed to [RuboCop](https://github.com/rubocop-hq/rubocop) — a great opportunity to explore how its AST-based static analysis engine operates under the hood. More than just a linter, RuboCop is a complete static analysis framework that parses Ruby source code and automatically corrects offenses against the [Ruby Style Guide](https://rubystyle.guide/).
...
To my surprise, RuboCop's autocorrection missed several occurrences. Rather than just reporting an issue, I decided to investigate the cop responsible for autocorrection - `Minitest/GlobalExpectations`. Its job is to rewrite deprecated global calls to Minitest's newer expectation DSL:
...
It attempted to **whitelist** acceptable receiver shapes, permitting only local/instance/class variables, zero-argument method calls, or simple call chains. But in Ruby's grammar, virtually any valid expression (constants, indexed lookups, chained invocations, block returns) can act as a method receiver.
...
Note the use of `[...]` in the `block_global_expectation?` pattern: in RuboCop's NodePattern DSL, `[...]` denotes a logical **AND** (all nested patterns must match), complementing `{...}` for logical **OR**.
