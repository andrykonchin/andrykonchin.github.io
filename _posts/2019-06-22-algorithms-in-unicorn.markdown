...
Это минимальный пример Rack-приложения, который по конвенции должен находиться в файле 'config.ru'. Согласно Rack-спецификации `application` должен быть _callable_ объект, который принимая данные о запросе будет возвращать ответ в виде массива из кода ответа, заголовков и _body_ (массива строк). Если быть точнее, то _body_ может быть любым объектом, не только массивом, который реализует метод `each`. При каждом вызове `each` должен возвращаться очередной фрагмент данных.
...
запускается сам веб-сервер. Метод `start` запустит _worker_'ы, а `join` будет обрабатывать управляющие системные сигналы и контролировать _worker_'ы до самого завершения работы.
