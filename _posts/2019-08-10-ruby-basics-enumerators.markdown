...
его заполнить. На этот раз объект изучения это класс `Enumerator`, его
связь с `Enumerable` и способы использования.
...
можно использовать и другие методы для итерации. Например, можно создать
...
=> [6, 4, 2]
```
Или, например, поработать не с элементами самой коллекции, а
пройтись по парам элементов сгенерированными методом `each_cons`:
...
В последнем примере раскрывается идея `Enumerator` - имея `Enumerable` и
метод для итерации (`each_cons`) можно создать другой `Enumerable`,
вычислимый и эфемерный. Ведь никакого массива пар `[[1, 2], [2, 3],
...]` не существует - каждый элемент последовательности вычисляется на
лету в методе `each_cons` при обходе.
...
Enumerator реализует метод `each` и получает из `Enumerable` производные от
each методы `cycle`, `each_entry`, `each_with_index`, `each_with_index`
и `reverse_each`. Они позволяют обойти коллекцию и обработать ее
элементы. Это можно назвать внутренним итератором. Ведь контроль за
итерациями остается у коллекции и клиент не управляет ни порядком обхода
коллекции и не выбирает момент, когда обработать очередной элемент:
...
`each_cons` и `each_slice`. Эти методы создают новую последовательность на
основе базовой коллекции и итерируют по ней. При этом эта
последовательность не создается в виде коллекции - она вычислима и
эфемерна.

Создать новую псевдо-последовательность можно вручную реализовав метод
аналогичный `each_cons` или используя `Enumerator`.
...
блок. Для примера реализуем метод аналогичный `Integer#times`:
...
Использованный ранее метод `to_enum` это всего лишь короткая запись для
следующего способа создания _enumerator_'а:
...
Псевдо-коллекция может быть бесконечной. Можно рассмотреть
такие коллекции на примере `Enumerable` и `Range`. Метод `cycle` может
обходить коллекцию по кругу бесконечно, а `Range` можно создать
бесконечным.
